...
class SolarSystemDataset(Dataset):
    def __init__(self, pos_path, mom_path, dpos_path, dmom_path, mode='train', train_ratio=0.8, stats=None):
        ...
        if mode == 'train':
            self.q_max = np.abs(q_raw).max(axis=0).astype('float64')
            self.p_max = np.abs(p_raw).max(axis=0).astype('float64')
            self.dq_max = np.abs(dq_raw).max(axis=0).astype('float64')
            self.dp_max = np.abs(dp_raw).max(axis=0).astype('float64')
        else:
