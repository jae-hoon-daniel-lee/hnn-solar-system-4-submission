...
from utils_data import import_mltsc_v2_data
...
BATCH_SZ = setting['batch'][set_idx]
...
PREV_EPOCS = -1
EXTRA_EPOCS = setting['epocs'][set_idx]

PLOT_ROOT_DIR="./figures"
TYPE_DIR=f"{PLOT_ROOT_DIR}/loss"
FIG_DIR=f"{TYPE_DIR}/multi-scale-v2"
...
def store_chkpt_for_insurance(epoch, loss, prefix, postfix, 
                              name=MODEL_NAME, model_dir=MODEL_DIR):
    save_dir = model_dir
    ...
    try:
        ...
        file_name = f"{prefix}{name}__chkpt4rescue{postfix}.pth"
...
def store_history_for_insurance(history, model_dir=MODEL_DIR):
    filename = f"stats_{MODEL_NAME}_" \
               f"_history4rescue_lr_{LR_INIT:.0e}" \
               f"_{MIN_LR}_pat_{PATIENCE}.pkl"
...
def prepare_all():
    ...
    train_loader, test_loader=import_mltsc_v2_data(model_dir=MODEL_DIR, 
                                                   data_dir_name='data_ode',
                                                   q_file_name='q_lite.npz', 
                                                   p_file_name='p_lite.npz',
                                                   dq_file_name='dq_lite.npz', 
                                                   dp_file_name='dp_lite.npz',
...
def train_with_val(start_epoch=start_epoch, num_extra_epochs=100):
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        store_history_for_insurance(history, MODEL_DIR)
        store_chkpt_for_insurance(epoch, avg_test_loss, OUTPUT_PREFIX, OUTPUT_POSTFIX, 
...
def process_post_train():
    ...
    end_idx = start_idx + len(history['train_losses']) - 1
    filename = f"stats_{MODEL_NAME}_" \
           f"ep_{start_idx}_to_{end_idx}_bs{BATCH_SZ}_lr_{LR_INIT:.0e}" \
           f"_{MIN_LR}_pat_{PATIENCE}.pkl"
...
if __name__== '__main__':
    ...
    start_idx = start_epoch + 1
    end_idx = start_idx + len(history['test_losses']) - 1
    filename = f"test-loss_{MODEL_NAME}_" \
               f"ep_{start_idx}_to_{end_idx}" \
               + OUTPUT_POSTFIX + ".webp"
    ...
    save_path = os.path.join(save_dir, filename)
    plot_losses(history['test_losses'], start_epoch, caption='test loss', save_path=save_path)
