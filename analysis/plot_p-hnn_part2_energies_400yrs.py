...
gt_energy_nn_x_filenames = ['p-hnn(4)(10)_gt_energy_nn_x_terms_part2_400_years.npz']
nn_energy_gt_x_filenames = ['p-hnn(4)(10)_nn_energy_gt_x_terms_part2_400_years.npz']
nn_energy_nn_x_filenames = ['p-hnn(4)(10)_nn_energy_nn_x_terms_part2_400_years.npz']
...
def plot_gt_energy_comparison(gt_T_gt_x, gt_V_gt_x, gt_H_gt_x,
    ...
    plt.title(f"GT Energies for P-HNN orbits (evaluation partition 2)", fontsize=16)
    plt.tight_layout()
    if img_save:
        save_path = f"{save_dir}/p-hnn-l4_gt_energy_comparison_part2_{start_pred_year}_{pred_time_years}_years.png"
...
def plot_nn_energy_comparison(nn_T_gt_x, nn_V_gt_x, nn_H_gt_x,
    ...
    plt.grid(True)
    plt.legend(loc='best', ncol=1, fontsize=10)
    plt.title(f"NN Energies of P-HNNs (evaluation partition 2)", fontsize=16)
    plt.tight_layout()
    if img_save:
        save_path = f"{save_dir}/p-hnn-l4_nn_energy_comparison_part2_{start_pred_year}_{pred_time_years}_years.png"
        plt.savefig(save_path, dpi=300, bbox_inches='tight')
    plt.show()    
...
plot_gt_energy_comparison(gt_Ts_gt_x, gt_Vs_gt_x, gt_Hs_gt_x,
                          gt_Ts_nn_x, gt_Vs_nn_x, gt_Hs_nn_x,
                          start_pred_year=3600, pred_time_years=400, 
                          steps_per_year=100,
                          full_test_years=400,
                          model_names=model_names,
